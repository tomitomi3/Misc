Namespace NormalDistribution
    ''' <summary>
    ''' Random number generator using Box-Muller method.(Normal Distribution)
    ''' </summary>
    ''' <remarks>
    ''' Refference:
    ...
    Public Class BoxMuller
        ...
        ''' <remarks>
        ''' </remarks>
        Public Function Random(Optional ByVal ai_ave As Double = 0, Optional ByVal ai_sigma2 As Double = 1) As Double
            Dim u1 As Double = Me.m_rand.NextDouble()
            Dim u2 As Double = Me.m_rand.NextDouble()

            Dim r As Double = Math.Sqrt(-2.0 * Math.Log(u1))
            Dim theta = 2.0 * Math.PI * u2
            ...
            If Me.flg = True Then
                Me.flg = False
                Return y1 * ai_sigma2 + ai_ave
            Else
                Me.flg = True
                Return y2 * ai_sigma2 + ai_ave
            End If
        End Function
